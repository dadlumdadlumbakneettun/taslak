function draw() {
    ...
    for (let col = startCol2; col <= endCol2; col++) {
        ...
        let rand = lcg(seed);
        if (rand() < 0.85) {
            let treeH = 140 + rand() * 130, treeW = 28 + rand() * 18;
    ...
    for (let col = startCol3; col <= endCol3; col++) {
        ...
        let rand = lcg(seed);
        if (rand() < 0.90) {
            let treeH = 180 + rand() * 140, treeW = 38 + rand() * 24;
    ...
    for (let col = startCol4; col <= endCol4; col++) {
        ...
        if (rand() < 0.70) {
            let treeH = 220 + rand() * 170, treeW = 55 + rand() * 30;
    ...
    ctx.fillRect(camera.x, 0, canvas.width, canvas.height - groundY);
    ctx.restore();
